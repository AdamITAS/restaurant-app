...
import { X, Plus, Minus, ShoppingBag, Search, Loader2, Receipt, ImageOff } from 'lucide-react'
...
export default function CustomerMenuView({ 
  ...
  // Controlla stato tavolo in tempo reale
  useEffect(() => {
  ...
  return (
    <main className="min-h-screen bg-slate-50 pb-32 relative">
      ...
      <header className="sticky top-0 bg-white/80 backdrop-blur-lg shadow-sm p-4 flex justify-between items-center z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs bg-slate-200 text-slate-600 px-2 py-0.5 rounded-full font-medium">
            {tableStatus === 'active' ? (lang === 'it' ? 'Occupato' : 'Active') : tableStatus === 'awaiting_payment' ? (lang === 'it' ? 'Da Pagare' : 'To Pay') : (lang === 'it' ? 'Libero' : 'Free')}
          </span>
        </div>
        <div className="flex gap-2 items-center">
          <button onClick={() => setLang(lang === 'it' ? 'en' : 'it')} className="p-2 rounded-full hover:bg-slate-100 text-lg transition">
            {lang === 'it' ? '🇬🇧' : '🇮🇹'}
          </button>
      ...
      {/* Filter Bar */}
      <div className="flex gap-2 p-4 overflow-x-auto bg-white border-b border-slate-100 scrollbar-hide">
        <button onClick={() => setFilterCategory(null)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${!filterCategory ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
          {t.all}
        </button>
        {categories.map(cat => (
          <button key={cat.id} onClick={() => setFilterCategory(cat.id)} className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all ${filterCategory === cat.id ? 'bg-slate-900 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            {getName(cat)}
          </button>
        ))}
      </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {items?.filter(item => item.is_top).map(item => (
                <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col hover:shadow-md transition-all">
                  {/* Placeholder per Immagine (Fase 4) */}
                  <div className="bg-slate-200 h-28 flex items-center justify-center text-slate-400">
                    <ImageOff size={24} />
                  </div>
                  <div className="p-3 flex flex-col flex-1">
                    ...
                    <div className="mt-3 flex justify-between items-center">
                      ...
                      <button onClick={() => addItem(item)} disabled={tableStatus === 'awaiting_payment'} className="bg-emerald-600 text-white w-8 h-8 rounded-full hover:bg-emerald-700 flex items-center justify-center transition active:scale-90 disabled:bg-slate-300">
                        <Plus size={18} />
                      </button>
                    </div>
                  </div>
                ...
                {categoryItems?.map(item => (
                  <div key={item.id} className="bg-white p-4 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center hover:shadow-md transition-all">
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      ...
                      <button onClick={() => addItem(item)} disabled={tableStatus === 'awaiting_payment'} className="bg-emerald-600 text-white w-9 h-9 rounded-full hover:bg-emerald-700 flex items-center justify-center transition active:scale-90 disabled:bg-slate-300 shadow-sm">
                        <Plus size={20} />
                      </button>
                    </div>
                  </div>
      ...
      {/* Floating Footer - Carrello o Conto */}
      {isHydrated && (
        <>
          ...
          {getTotalItems() > 0 && tableStatus !== 'awaiting_payment' && (
            <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-slate-200 p-4 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              <div className="max-w-3xl mx-auto flex justify-between items-center">
                <div>
                  <p className="font-bold text-slate-800 text-sm">{getTotalItems()} {t.items}</p>
                  <p className="text-emerald-600 font-bold text-2xl tracking-tight">€{getTotalPrice().toFixed(2)}</p>
                </div>
                <button onClick={toggleCart} className="bg-slate-900 text-white px-8 py-3.5 rounded-xl font-bold hover:bg-slate-800 shadow-lg transition active:scale-95">
                  {t.viewCart}
                </button>
              </div>
            </div>
          ...
          {/* Pulsante Fisso Chiedi Conto (Carrello vuoto e tavolo occupato) */}
          {tableStatus === 'active' && getTotalItems() === 0 && (
            <div className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-lg border-t border-purple-200 p-4 z-20 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
              <div className="max-w-3xl mx-auto">
                <button onClick={handleRequestBill} className="w-full bg-purple-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-purple-700 shadow-lg transition active:scale-95 flex items-center justify-center gap-2">
            ...
            {cartItems.length > 0 && (
              <div className="p-5 border-t bg-white space-y-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                <div className="flex justify-between items-center">
                  <p className="text-slate-500 font-semibold">{t.total}</p>
                  <p className="text-3xl font-bold text-slate-900 tracking-tight">€{getTotalPrice().toFixed(2)}</p>
                </div>
                <button 
                  onClick={handleConfirmOrder} 
                  disabled={isSubmitting}
                  className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-emerald-700 transition shadow-lg disabled:bg-slate-300 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-95"
                >
